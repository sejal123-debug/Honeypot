    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if cmd == "USER":
                ...
                # Special alert for users.txt download
                if arg.lower() == "users.txt" or full_path.lower().endswith("/users.txt"):
                    _log(f"🚨 RED ALERT: SENSITIVE FILE DOWNLOAD - {self.username} ({_ftp_get_privilege_name(self.user_privilege)}) downloaded users.txt from {full_path} - USER CREDENTIALS EXPOSED! 🚨", level="ALERT")
                    _structured_log("SENSITIVE_FILE_DOWNLOAD", {
                        "file": "users.txt",
                        "full_path": full_path,
                        "operation": "download",
                        "privilege": _ftp_get_privilege_name(self.user_privilege),
                        "severity": "HIGH",
                        "description": "User credentials file downloaded - potential credential harvesting attempt"
                    }, self.addr[0], self.username, self.session_id)
