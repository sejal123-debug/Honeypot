...
def _save_downloaded_file(filename: str, content: bytes, username: str) -> Optional[str]:
    """
    Save downloaded file to real filesystem for analysis
    Returns the path where file was saved, or None if failed
    """
    try:
        # Create downloads directory if it doesn't exist
        downloads_dir = "ftp_downloads"
        ...
        # Log the file save
        _log(f"Downloaded file saved: {file_path} ({len(content)} bytes)", level="INFO")
        
        return file_path
    except Exception as e:
        _log(f"Failed to save downloaded file {filename}: {e}", level="ERROR")
        return None
...
def _check_ftp_file_access(user_privilege: int, file_path: str, operation: str, username: str) -> tuple[bool, str]:
    ...
    # Special case: Restrict root directory (/) access to ROOT users only
    if file_path == "/" and user_privilege < _FTP_PRIVILEGE_LEVELS["root"]:
        _log(f"FTP_ACCESS_DENIED: {username} ({_ftp_get_privilege_name(user_privilege)}) attempted {operation} on root directory (/) - ROOT ACCESS REQUIRED", level="WARNING")
        return False, "Root directory access restricted to root users only"
    
    # Special case: Allow ALL users (guest, user, admin, root) to access users.txt
    filename = os.path.basename(file_path).lower()
    if filename == "users.txt":
        _log(f"FTP_USERS_TXT_ACCESS: {username} ({_ftp_get_privilege_name(user_privilege)}) accessing users.txt - ALLOWED for all users", level="INFO")
        return True, "users_txt_allowed_for_all_users"
    ...
    if user_privilege == _FTP_PRIVILEGE_LEVELS["guest"]:
        if not file_path.startswith("/ftp") and file_path != "/ftp":
    ...
    if user_privilege == _FTP_PRIVILEGE_LEVELS["user"]:
        if not (file_path.startswith("/ftp") or file_path.startswith("/home") or file_path == "/ftp" or file_path == "/home"):
    ...
    # Admin users can access most areas except root-only paths and root directory (already checked above)
    if user_privilege >= _FTP_PRIVILEGE_LEVELS["admin"]:
...
class _FTPClientHandler(threading.Thread):
    ...
    def _monitor_data_transfer(self, filename: str, file_size: int, operation: str) -> None:
        """Monitor and log data transfer activities"""
        try:
            # Log the transfer
            _log(f"DATA_TRANSFER: {operation} {filename} ({file_size} bytes) by {self.username} from {self.peer}", level="INFO")
            
            # Create structured log entry
            _structured_log("DATA_TRANSFER", {
                "filename": filename,
                "file_size": file_size,
                "operation": operation,
            ...
            suspicious_extensions = ['.php', '.jsp', '.asp', '.exe', '.bat', '.sh', '.py', '.pl']
            file_ext = os.path.splitext(filename)[1].lower()
            
            if file_ext in suspicious_extensions:
                _log(f"🚨 SUSPICIOUS FILE TRANSFER: {operation} {filename} ({file_ext}) by {self.username} - POTENTIAL MALWARE UPLOAD! 🚨", level="ALERT")
                _structured_log("SUSPICIOUS_FILE_TRANSFER", {
                    "filename": filename,
                    "file_extension": file_ext,
                    "operation": operation,
                    "file_size": file_size,
                    "severity": "HIGH",
                    "description": "Suspicious file type transfer detected"
                }, self.addr[0], self.username, self.session_id)
            
            # Check for large file transfers
            if file_size > 10 * 1024 * 1024:  # 10MB
                _log(f"⚠️ LARGE FILE TRANSFER: {operation} {filename} ({file_size} bytes) by {self.username} - POTENTIAL DATA EXFILTRATION! ⚠️", level="WARNING")
                _structured_log("LARGE_FILE_TRANSFER", {
                    "filename": filename,
                    "file_size": file_size,
                    "operation": operation,
                    "severity": "MEDIUM",
                    "description": "Large file transfer detected"
                }, self.addr[0], self.username, self.session_id)
                
        except Exception as e:
            _log(f"Error monitoring data transfer: {e}", level="ERROR")

    def _analyze_file_content(self, filename: str, content: bytes, operation: str) -> None:
        """Analyze file content for security monitoring"""
        try:
            content_str = content.decode('utf-8', errors='ignore')
            
            # Check for sensitive patterns
            sensitive_patterns = [
                ('password', 'Password found in file'),
                ('secret', 'Secret found in file'),
                ('key', 'Key found in file'),
                ('token', 'Token found in file'),
                ('credential', 'Credential found in file'),
                ('admin', 'Admin reference found in file'),
                ('root', 'Root reference found in file'),
                ('mysql', 'Database reference found in file'),
                ('database', 'Database reference found in file'),
                ('api_key', 'API key found in file'),
                ('private_key', 'Private key found in file'),
                ('ssh', 'SSH reference found in file'),
                ('ftp', 'FTP reference found in file'),
                ('login', 'Login reference found in file'),
                ('auth', 'Authentication reference found in file')
            ]
            
            for pattern, description in sensitive_patterns:
                if pattern.lower() in content_str.lower():
                    _log(f"🔍 CONTENT ANALYSIS: {description} in {filename} during {operation} by {self.username}", level="WARNING")
                    _structured_log("SENSITIVE_CONTENT_DETECTED", {
                        "filename": filename,
                        "pattern": pattern,
            ...
            if content_str.startswith('#!/'):
                _log(f"📄 SCRIPT FILE: {filename} appears to be a script file during {operation} by {self.username}", level="INFO")
            elif content_str.startswith('-----BEGIN'):
                _log(f"🔐 CRYPTOGRAPHIC FILE: {filename} appears to contain cryptographic data during {operation} by {self.username}", level="WARNING")
            elif '<?php' in content_str or '<?=' in content_str:
                _log(f"🐘 PHP FILE: {filename} appears to be a PHP file during {operation} by {self.username}", level="WARNING")
            elif '<html' in content_str or '<HTML' in content_str:
                _log(f"🌐 HTML FILE: {filename} appears to be an HTML file during {operation} by {self.username}", level="INFO")
                
        except Exception as e:
            _log(f"Error analyzing file content: {e}", level="ERROR")
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if cmd == "USER":
                ...
                allowed, reason = _check_ftp_file_access(self.user_privilege, full_path, "read", self.username)
                if not allowed:
                    self.send("550 Permission denied.")
                    _structured_log("FTP_FILE_ACCESS", {
                        "operation": "read",
                ...
                # Special alert for users.txt download (now accessible to all users)
                if arg.lower() == "users.txt" or full_path.lower().endswith("/users.txt"):
                    _log(f"📋 USERS.TXT DOWNLOAD - {self.username} ({_ftp_get_privilege_name(self.user_privilege)}) downloaded users.txt from {full_path} - ACCESSIBLE TO ALL USERS", level="INFO")
                    _structured_log("USERS_TXT_DOWNLOAD", {
                        "file": "users.txt",
                        "full_path": full_path,
                        "operation": "download",
                        "privilege": _ftp_get_privilege_name(self.user_privilege),
                        "severity": "MEDIUM",
                        "description": "User file downloaded - accessible to all user types"
                    }, self.addr[0], self.username, self.session_id)
                ...
                # Analyze file content for security monitoring
                self._analyze_file_content(arg, payload, "DOWNLOAD")
                ...
                allowed, reason = _check_ftp_file_access(self.user_privilege, full_path, "delete", self.username)
                if not allowed:
                    self.send("550 Permission denied.")
                    _structured_log("FTP_FILE_ACCESS", {
                        "operation": "delete",
