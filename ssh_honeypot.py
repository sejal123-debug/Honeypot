...
_USER_PRIVILEGES = {
    ...
    "guest": "guest",
    "admin": "Admin@2024!",
    "root": "Root#Secure$2024",
    "john.doe": "J0hnD03!@#2024",
    "jane": "J@n3$ecur3P@ss",
    "mike": "M1k3!C0mpl3x#2024",
    "sarah": "S@r@h$tr0ng!2024",
    "david": "D@v1d#C0rpP@ss",
    "lisa": "L1s@!Ent3rpr1s3",          
    "robert": "R0b3rt$C0mpl3x",
    "amanda": "A@m@nd@!S3cur3",
    "chris": "Chr1s#C0rp2024",
    "jennifer": "J3nn!f3r$tr0ng",
    "ftp": "Ftp@S3rv3r!2024",
    "backup": "B@ckup#S3cur3$2024",
    "monitor": "M0n1t0r!@l3rt",
}
...
def _check_file_access(user_privilege: int, file_path: str, operation: str, username: str) -> tuple[bool, str]:
    ...
    if file_path.startswith("/home/"):
        ...
        if len(path_parts) >= 3:
            ...
            # Special case: Only mike user can access /home/mike (restricted access)
            elif target_user == "mike":
                if username == "mike":
                    return True, "mike_own_home_access"
                else:
                    _log(f"MIKE_HOME_ACCESS_DENIED: {username} attempted to access {file_path} - only mike user allowed", level="WARNING")
                    return False, "access_denied_mike_home_restricted_to_mike_user_only"
            ...
            # Admin and root can access all user homes EXCEPT mike's home
            elif user_privilege >= _PRIVILEGE_LEVELS["admin"]:
                if target_user == "mike":
                    _log(f"MIKE_HOME_ADMIN_ACCESS_DENIED: {username} (admin/root) attempted to access {file_path} - mike home restricted", level="WARNING")
                    return False, "access_denied_mike_home_restricted_even_from_admin_root"
                else:
...
def _handle_client(client: socket.socket, addr: Tuple[str, int], host_key: paramiko.PKey,
    ...
    try:
                ...
                "children": {
                    ...
                    "home": {"type": "dir", "children": {
                        "john": {"type": "dir", "children": {
                            ...
                            "key.txt": {"type": "file", "content": "flag_1=Flag{user_temp2025}"},
                        ...
                        "mike": {"type": "dir", "children": {
                            ".bash_history": {"type": "file", "content": "sudo -l\r\nwhoami\r\nls -la\r\ncat credentials.txt"},
                            "credentials.txt": {"type": "file", "content": f"database_user:db_secret_key\r\nFlag_2:Flag1234567890abcdef"},
                            "notes.txt": {"type": "file", "content": "Welcome Mike, Some files may “borrow” the powers of their owners... maybe one of them can help you. "},
                            ".ssh": {"type": "dir", "children": {
                                "id_rsa": {"type": "file", "content": "-----BEGIN OPENSSH PRIVATE KEY-----\r\nb3BlbnNzaC1rZXktdjEAAAAABG5vbmUAAAAEbm9uZQAAAAAAAAABFwAAAAdzc2gtcn\r\nNhAAAAAwEAAQAAAQEA...MIKE_PRIVATE_KEY...\r\n-----END OPENSSH PRIVATE KEY-----"},
                                "authorized_keys": {"type": "file", "content": "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQ... mike@server"}
                            }}
                        }},
                    ...
                    "tmp": {"type": "dir", "children": {
                        "pass.txt": {"type": "file", "content": "ZHVtYnN0cnVjawo=", "perms": "-rw-r--r--", "owner": "root", "group": "root", "size": 10},
                    }},
                    ...
                    "root": {"type": "dir", "children": {".bash_history": {"type": "file", "content": "sudo su\r\nwhoami"}, "flag_3.txt": {"type": "file", "content": "DYNAMIC_FLAG"}, ".ssh": {"type": "dir", "children": {"authorized_keys": {"type": "file", "content": "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQD... user@host"}}}}},
        ...
        while True:
            ...
            if ch in (10, 13):
                ...
                if cmd in {"python3"}:
                    ...
                    # Only works for mike user
                    if ("os.execl" in line and "/bin/sh" in line and "sh" in line and "-p" in line):
                        if user_display == "mike":
                            # Simulate successful privilege escalation for mike user only
                            _log(f"PRIVILEGE_ESCALATION_SUCCESS: {user_display} gained root via Python3 SUID exploit", level="ALERT")
                            _log(f"ROOT_ESCALATION: mike user successfully escalated to root using Python3 SUID exploit")
                            user_display = "root"
                            ...
                            # Other users cannot use this exploit
                            _log(f"PRIVILEGE_ESCALATION_DENIED: {user_display} attempted Python3 SUID exploit but only mike can do this", level="WARNING")
                            chan.send(b"python3: Permission denied\r\n")
                            chan.send(_prompt())
                            continue
                ...
                # User privilege
                    ...
                    if not args:
                        ...
                        # Special case: john user can run commands as mike without password
                        if user_display == "john":
                            chan.send(b"    (mike) NOPASSWD: ALL\r\n")
                            _log(f"SUDO_PRIVILEGE_ESCALATION: john can run commands as mike without password")
                        else:
                        ...
                        if target_user in ["admin", "root"]:
                            ...
                            target_privilege = _USER_PRIVILEGE_LEVELS.get(target_user, "user")
                            server.user_privilege = _PRIVILEGE_LEVELS.get(target_privilege, _PRIVILEGE_LEVELS["user"])
                    elif args[0] == "-u" and len(args) >= 3 and args[1] == "mike" and "/bin/bash" in " ".join(args[2:]):
                        # Special case: sudo -u mike /bin/bash -p (privilege escalation from john to mike)
                        if user_display == "john":
                            _log(f"SUDO_PRIVILEGE_ESCALATION: john attempted sudo -u mike /bin/bash -p")
                            _log(f"SUDO_PRIVILEGE_ESCALATION_SUCCESS: john successfully switched to mike user via sudo -u mike /bin/bash -p")
                            
                            # Switch to mike user
                            user_display = "mike"
                            target_privilege = _USER_PRIVILEGE_LEVELS.get("mike", "user")
                            server.user_privilege = _PRIVILEGE_LEVELS.get(target_privilege, _PRIVILEGE_LEVELS["user"])
                            
                            chan.send(b"Switched to user mike\r\n")
                            chan.send(_prompt())
                            continue
                        else:
                            # Other users cannot use this privilege escalation
                            _log(f"SUDO_PRIVILEGE_ESCALATION_DENIED: {user_display} attempted sudo -u mike /bin/bash -p but only john can do this")
                            chan.send(b"sudo: user not allowed to run /bin/bash as mike\r\n")
                            chan.send(_prompt())
                            continue
                    else:
                ...
                #         chan.send(("\r\n".join(lines) + "\r\n").encode())
                    ...
                    if not args:
                        ...
                        # Access control is handled by _resolve function
                        ###########
                        output_parts = []
                        for path in args:
                            ...
                            if absolute_path == "/var/log/auth.log":
                                output_parts.append("\n".join(_AUTH_EVENTS))
                                continue
                            if absolute_path == "/root/flag_3.txt":
                                output_parts.append(
                                    f"key:flagroot_privilege_escalation\r\n"
                                    f"\nTimestamp: {datetime.utcnow().isoformat()}Z"
                                )
                                continue
                            
                            # Special logging for mike's credential files
                            if absolute_path.startswith("/home/mike/") and user_display == "mike":
                                filename = os.path.basename(absolute_path)
                                if filename in ["credentials.txt", "config.ini", "backup_keys.txt", "notes.txt"]:
                                    _log(f"🔐 MIKE_CREDENTIALS_ACCESS: mike user accessed sensitive credential file: {filename}", level="ALERT")
                                    # Continue to normal file processing to display contents
                            ...
                            if node and node.get("type") == "file":
                                ...
                                # Access denied by _resolve function
        ...
        import traceback
        _log(f"SSH session traceback: {traceback.format_exc()}")
    finally:
...
def start_ssh_honeypot(address: str, port: int, username: Optional[str], password: Optional[str]) -> None:
    ...
    if username or password:
        ...
        _log("[i] Accepting password credential")
