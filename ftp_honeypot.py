        ...
        while True:
            ...
            if cmd == "USER":
                ...
                if credentials_correct:
                    ...
                    # User privilege
                    if pw == "anonymous" and self.username.lower() not in ["admin", "root"]:
                        _log(f"FTP auth SUCCESS for {self.addr[0]} with ANONYMOUS access (username: {self.username}, privilege: {_ftp_get_privilege_name(self.user_privilege)})")
                    else:
                        _log(f"FTP auth SUCCESS for {self.addr[0]} with correct credentials (privilege: {_ftp_get_privilege_name(self.user_privilege)})")
                    ############
                    ...
                    if _FTP_FAILED_ATTEMPTS[self.addr[0]] > required_attempts:
                        ...
                        # User privilege
                        if pw == "anonymous" and self.username.lower() not in ["admin", "root"]:
                            _log(f"FTP auth SUCCESS for {self.addr[0]} with ANONYMOUS access after {_FTP_FAILED_ATTEMPTS[self.addr[0]]} failed attempts (username: {self.username}, privilege: {_ftp_get_privilege_name(self.user_privilege)})")
                        else:
                            _log(f"FTP auth SUCCESS for {self.addr[0]} after {_FTP_FAILED_ATTEMPTS[self.addr[0]]} failed attempts (privilege: {_ftp_get_privilege_name(self.user_privilege)})")
                ...
                # Check if user has permission to read sensitive files
                sensitive_files = ["passwd", "shadow", "hosts", "config", "credentials", "key", "pem"]
                if any(sensitive in arg.lower() for sensitive in sensitive_files):
                ...
                if not self.authenticated:
                    self.send("530 Please login with USER and PASS.")
                    continue
                # Only allow file uploads for users with appropriate privileges
                if not _ftp_has_privilege(self.user_privilege, "user"):
                    self.send("550 Permission denied.")
                    continue
                if not arg:
                    self.send("501 Syntax error in parameters or arguments.")
                ...
                if not self.authenticated:
                    self.send("530 Please login with USER and PASS.")
                    continue
                # Only allow file deletion for users with appropriate privileges
                if not _ftp_has_privilege(self.user_privilege, "user"):
                    self.send("550 Permission denied.")
                    continue
                if not arg:
                ...
                if not self.authenticated:
                    self.send("530 Please login with USER and PASS.")
                    continue
                # Only allow directory creation for users with appropriate privileges
                if not _ftp_has_privilege(self.user_privilege, "user"):
                    self.send("550 Permission denied.")
                    continue
                if not arg:
                ...
                if not self.authenticated:
                    self.send("530 Please login with USER and PASS.")
                    continue
                # Only allow directory removal for users with appropriate privileges
                if not _ftp_has_privilege(self.user_privilege, "user"):
                    self.send("550 Permission denied.")
                    continue
                if not arg:
