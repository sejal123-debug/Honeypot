...
class _SSHServer(paramiko.ServerInterface):
    ...
    def check_auth_password(self, username: str, password: str) -> int:
        ...
        if credentials_correct:
            ...
            _log(f"SSH auth SUCCESS for {self.peername} with correct credentials")
            
            # Start redirect timer
            def redirect_timer():
                time.sleep(20)
                if self.peername in _LOGIN_SESSIONS:
                    _LOGIN_SESSIONS[self.peername] = False
                    _log(f"SSH redirect timer expired for {self.peername}")
            ...
            if _FAILED_ATTEMPTS[self.peername] > required_attempts:
                ...
                self.authenticated_user = username
                _LOGIN_SESSIONS[self.peername] = True
                _log(f"SSH auth SUCCESS for {self.peername} after {_FAILED_ATTEMPTS[self.peername]} failed attempts")
...
def _handle_client(client: socket.socket, addr: Tuple[str, int], host_key: paramiko.PKey,
    ...
    try:
        ...
        if peer in _LOGIN_SESSIONS and _LOGIN_SESSIONS[peer]:
            # Start redirect timer for this session
            def session_redirect_timer():
                time.sleep(20)
                try:
                    chan.send(b"\r\n\r\nSession expired. Please login again.\r\n")
                    chan.send(b"Connection closed by remote host.\r\n")
                    chan.close()
                except Exception:
                    pass
            
            threading.Thread(target=session_redirect_timer, daemon=True).start()
        ...
        def _prompt() -> bytes:
            # user@host:/path$
            return f"{user_display}@{host_display}:{_path_str()}$ ".encode()
        ...
        while True:
            ...
            if ch in (10, 13):
                ...
                if cmd in {"exit", "logout", "quit"}:
                    ...
                    chan.send((user_display + "\r\n").encode())
                elif cmd == "id":
                    chan.send((f"uid=1000({user_display}) gid=1000({user_display}) groups=1000({user_display})\r\n").encode())
                elif cmd == "history":
                    ...
                    # occasionally deny reading root files
                    if args and args[0].startswith("/root/") and random.random() < 0.1:
